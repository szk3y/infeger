...
void copy_list(List* origin, List* clone); // clone = origin; 順番逆にしたい
bool is_empty(List*);
...
bool has_prev_prev_node(Node*); // node->prev->prev != NULLを返す．ぬるぽ防止
bool has_next_next_node(Node*); // node->next->next != NULLを返す．ぬるぽ防止
